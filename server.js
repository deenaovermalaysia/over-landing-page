...
function parseTikTokData(rows) {
  // Row 1 (index 0): TOTAL Revenue — scan A:H for RM value
  // Row 2 (index 1): TOTAL Profit  — scan A:H for RM value
  let totalRevenue = 0, totalProfit = 0;
  (rows[0] || []).forEach(cell => {
    const s = String(cell || '').trim();
    if (s.startsWith('RM') && !totalRevenue) totalRevenue = extCleanNum(s);
  });
  (rows[1] || []).forEach(cell => {
    const s = String(cell || '').trim();
    if (s.startsWith('RM') && !totalProfit) totalProfit = extCleanNum(s);
  });

  // Row 4+ (index 3+): daily data
  // Cols: A=Date, B=TikTok Sales, C=Ads Cost, D=COGS, E=Platform Fees,
  //       F=Live Host Cost (Total Spending), G=KOL Fees, H=Total Profit (Excl. Op)
  const daily = [];
  for (let i = 3; i < rows.length; i++) {
    const row = rows[i] || [];
    const ds = String(row[0] || '').trim();
    if (!ds || !/\d{1,2}\/\d{2}\/\d{4}/.test(ds)) continue;
    const sales = extCleanNum(row[1]);
    if (sales === 0 && !row[2]) continue; // skip empty rows
    daily.push({
      date:     ds,
      sales:    sales,
...
app.get('/api/tiktok/tabs', requireAuth, async (req, res) => {
  try {
    ...
    // Match "MAY 2026", "JANUARY 2026" etc. — full uppercase month + year
    const MONTHS_UPPER = ['JANUARY','FEBRUARY','MARCH','APRIL','MAY','JUNE',
                          'JULY','AUGUST','SEPTEMBER','OCTOBER','NOVEMBER','DECEMBER'];
    const MONTHS_FULL  = ['January','February','March','April','May','June',
                          'July','August','September','October','November','December'];

    const MONTH_PATTERN = /^([A-Z]+)\s+(\d{4})$/;
    const availMonths = [];
    allTabNames.forEach(t => {
      const m = t.match(MONTH_PATTERN);
      if (!m) return;
      const mIdx = MONTHS_UPPER.indexOf(m[1]);
      if (mIdx < 0) return;
      availMonths.push({ tab: t, month: MONTHS_FULL[mIdx], year: m[2] });
    });

    availMonths.sort((a, b) => {
      const byYear = parseInt(b.year) - parseInt(a.year);
      if (byYear !== 0) return byYear;
      return MONTHS_FULL.indexOf(b.month) - MONTHS_FULL.indexOf(a.month);
    });
...
app.get('/api/tiktok', requireAuth, async (req, res) => {
  try {
    ...
    // Fetch rows 1-2 (totals) + row 3 (headers) + daily: A1:H35
